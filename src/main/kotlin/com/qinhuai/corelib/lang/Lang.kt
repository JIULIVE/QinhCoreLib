package com.qinhuai.corelib.lang
...
object Lang {
    ...
    fun load() {
        ...
        if (active.isEmpty() && locale != FALLBACK_LOCALE) {
            plugin.logger.warning("[QinhCoreLib] language '$locale' has no lang files under lang/$locale/, falling back to $FALLBACK_LOCALE")
        }
    }
    ...
    fun log(message: String) {
        TextUtil.logColored(QinhCoreLib.instance, message)
    }

    private fun loadLocaleInto(plugin: QinhCoreLib, loc: String, into: MutableMap<String, String>) {
    ...
    private fun saveBundledDefaults(plugin: QinhCoreLib) {
